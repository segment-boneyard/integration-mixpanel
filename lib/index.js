...
Mixpanel.prototype.page = function(msg, fn){
  ...
  var genericPageName = this.settings.genericPageName;
  ...
  if (genericPageName){
    //  but still respect their other settings
    if (trackCategorizedPages && category) return send(msg.track(), fn);
    if (trackNamedPages && name) return send(msg.track(), fn);
  }
